...
<script type="text/javascript">
	...
	function deleteLink(){
		...
		if(selectedRows.length==0){
			$.messager.alert("系统提示","请选择要删除的友情链接数据！");
			return;
		}
		var strIds=[];
		...
		var ids=strIds.join(",");
		$.messager.confirm("系统提示","您确定要删除这<font color=red>"+selectedRows.length+"</font>条友情链接数据吗？",function(r){
			if(r){
				$.post("${pageContext.request.contextPath}/admin/link/delete.do",{ids:ids},function(result){
					if(result.success){
						if(result.exist){
							...
							$.messager.alert("系统提示","友情链接数据已成功删除！");							
						}
						$("#dg").datagrid("reload");
					}else{
						$.messager.alert("系统提示","友情链接数据删除失败！");
					}
				},"json");
			}
		});
...
<table id="dg" title="友情链接管理" class="easyui-datagrid" 
  ...
  <thead>
  	<tr>
  		...
  		<th field="linkUrl" width="100" align="center">链接地址</th>
  		<th field="orderNo" width="100" align="center">链接排序</th>
  	</tr>
  </thead>
</table>
<div id="tb">
...
<div id="dlg" class="easyui-dialog" style="width: 550px;height: 230px;padding: 10px 20px" closed="true" buttons="#dlg-buttons">
	<form id="fm" method="post">
		<table cellspacing="8px">
			...
			<tr>
				<td>友情链接地址：</td>
				<td>
					<input type="text" id="linkUrl" name="linkUrl" class="easyui-validatebox"  validType="url" required="true" style="width: 170px"/>
				</td>
			</tr>
			<tr>
				<td>友情链接排序：</td>
				<td>
					<input type="text" id="orderNo" name="orderNo" class="easyui-numberbox" required="true"/>&nbsp;(类别根据排序序号从小到大排序)
				</td>
			</tr>
		</table>
	</form>
