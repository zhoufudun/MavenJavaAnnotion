					...
					series : [
						{
							...
							data:[
								 {name:"制品数",value:'${addOrder[0].finishNumber}'},
							     {name:"次品数",value:'${addOrder[0].xDefectiveNum+addOrder[0].zDefectiveNum+addOrder[0].yDefectiveNum}'}
							   ]
						}
					]
				};
		  		...
		  		<table  width="100%" height="500" align="center" border="0" cellpadding="0" cellspacing="2" style="border-radius:0px;">
					<tr>
						<td bgcolor="#F6F6F6" align="center" >
							<div  style="height:320px; border:solid 1px white; width:95%;border-radius:8px 8px 8px 8px; background-color:white;">	
								<div style="height:20px;">
									<div style="float:left; padding-left:15px;"><img src="${pageContext.request.contextPath}/static/images/15.png"></div>	
									<div style="font-size:16px;font-weight:bold;float:left;margin-top:3px;">某工单详情</div>	
					...
					<tr  style="height:40px;">
						<td align="center">
							<div  style="height:30px; border:solid 1px white; width:95%;border-radius:4px 4px 4px 4px; background-color:white;">	
								...
								<label id="label2"  style="float:right;font-size:16px;">${addOrder[0].orderNumber}</label>	
							</div>
						</td>
					</tr>
					...
					<tr  style="height:40px;">
						<td align="center">
							<div  style="height:30px; border:solid 1px white; width:95%;border-radius:4px 4px 4px 4px; background-color:white;">	
								<label id="label1"  style="float:left; font-size:16px;">状态</label>	
								<label id="label2"  style="float:right;font-size:16px;">
									<c:choose>
										<c:when test="${addOrder[0].machine!=null}">
												<c:if test="${addOrder[0].machine.state==0}">
													<font color="green">订单完成</font>
												</c:if>											
												<c:if test="${addOrder[0].machine.state==1}">
													<font color="blue">运行中</font>
												</c:if>
												<c:if test="${addOrder[0].machine.state==2}">
													<font color="red">停止</font>
												</c:if>
												<c:if test="${addOrder[0].machine.state==3}">
													<font color="red">修理</font>
												</c:if>
												<c:if test="${addOrder[0].machine.state==4}">
													<font color="red">保养</font>
												</c:if>
												<c:if test="${addOrder[0].machine.state==5}">
													<font color="red">报废</font>
												</c:if>
												<c:if test="${addOrder[0].machine.state==6}">
													派工未执行
												</c:if>											
										</c:when>
										<c:otherwise> 
											<c:choose>
												<c:when test="${addOrder[0].historyMacNum!=null}">
													<font color="blue">工单完成</font>									
												</c:when>
												<c:otherwise> 
													<font color="red">未分派机器</font>
												</c:otherwise>	
											</c:choose>																																								
										</c:otherwise>
									</c:choose>
					...
					<tr  style="height:40px;">
						<td align="center">
							<div  style="height:30px; border:solid 1px white; width:95%;border-radius:4px 4px 4px 4px; background-color:white;">	
								<label id="label1"  style="float:left; font-size:16px;">完成率</label>	
								<label id="label2"  style="float:right;font-size:16px;">${addOrder[0].finishNumber}/${addOrder[0].number}</label>	
							</div>
						</td>
					</tr>
					...
					<tr style="height:40px;">
						<td align="center">
							<div  style="height:30px; border:solid 1px white; width:95%;border-radius:4px 4px 4px 4pxs; background-color:white;">	
								<label id="label3"  style="float:left; font-size:16px;">次品率</label>	
								<label id="label4"  style="float:right;font-size:16px;">${addOrder[0].xDefectiveNum+addOrder[0].zDefectiveNum+addOrder[0].yDefectiveNum}/${addOrder[0].finishNumber}</label>			
							</div>
						</td>
					</tr>
					...
					<tr style="height:40px;">
						<td align="center">
							<div  style="height:30px; border:solid 1px white; width:95%;border-radius:4px 4px 4px 4pxs; background-color:white;">
									<c:choose>
										<c:when test="${addOrder[0].machine==null}">
											<label id="label3"  style="float:left; font-size:16px;">历史机器编号</label>									
								            <label id="label4"  style="float:right;font-size:16px;">${addOrder[0].historyMacNum}</label>																							
										</c:when>
										<c:otherwise> 
											<label id="label3"  style="float:left; font-size:16px;">生产线编号/机器编号</label>									
								            <label id="label4"  style="float:right;font-size:16px;">${addOrder[0].machine.lineNumber}/${addOrder[0].macNum}</label>																																						
										</c:otherwise>
									</c:choose>	
							</div>
						</td>
